...
def find_runs(root: Path) -> list[dict]:
    ...
    for status_path in root.rglob("status.json"):
        ...
        if len(chars) != 2 or len(seeds) != 2:
            continue
        rounds1 = read_jsonl(p1)
        rounds2 = read_jsonl(p2)
        ...
        runs.append(
            {
                "dir": run_dir,
                "status": status,
                "characters": [str(x) for x in chars],
...
def summarize(runs: list[dict]) -> tuple[list[dict], dict]:
    # Mirrored P1/P2 jobs with the same character-specific RNG seeds are exact
    # duplicate experimental conditions. Count them as raw evidence but keep one
    # representative for independent-condition metrics.
    reps = {}
    ...
    independent_nonzero = sum(
        reward_nonzero(x) for r in representative for x in r["p1"]
    )

    actions = collections.Counter()
    character_stats = collections.defaultdict(
        lambda: {
            "rounds": 0,
            "wins": 0,
            "losses": 0,
    ...
    metrics = {
        ...
        "independent_nonzero_reward_rate": (
            independent_nonzero / independent_rounds if independent_rounds else 0.0
        ),
        "total_decisions": total_decisions,
        "action_counts": {
            ACTION_NAMES.get(k, str(k)): int(v) for k, v in sorted(actions.items())
        },
        "action_rates": {
            ACTION_NAMES.get(k, str(k)): (
                v / total_decisions if total_decisions else 0.0
            )
        ...
        "characters": {
            character: {
                ...
                "action_counts": {
                    ACTION_NAMES.get(k, str(k)): int(n)
                    for k, n in sorted(value["actions"].items())
                },
            }
...
def compact_brain(transition: dict) -> dict:
...
def build_replay(representative: list[dict]) -> dict:
    # Prefer a round with a real HP difference so the public replay shows an
    # actual game consequence. Fall back to the latest available draw.
    candidate = None
    for run in representative:
    ...
    for t1 in round1.get("transitions", []):
        ...
        d1 = display.get("p1") or {}
        d2 = display.get("p2") or {}
        frames.append(
            {
                "frame": frame,
                "p1": {
                    **{k: d1.get(k) for k in ("hp", "x", "y", "speed_x", "speed_y")},
                    "action": ACTION_NAMES.get(int(t1["action"]), str(t1["action"])),
                    "brain": compact_brain(t1),
                },
                "p2": {
                    **{k: d2.get(k) for k in ("hp", "x", "y", "speed_x", "speed_y")},
                    "action": ACTION_NAMES.get(int(t2["action"]), str(t2["action"])),
                    "brain": compact_brain(t2),
                },
            }
        )
...
def main() -> int:
    ...
    if not runs:
        raise SystemExit("No canonical run artifacts found")
    representative, metrics = summarize(runs)
    replay = build_replay(representative)
    ...
    status = {
        ...
        "canonical_model": "MaleCNS v1.0 + pinned Shiu LIF",
        "randomness": (
            "observation-driven Poisson sensory spikes; deterministic "
            "spike-count argmax readout"
        ),
        "reward_design": {
            "id": "R0-terminal-hp-sign",
            "win": 1.0,
            "draw": 0.0,
            "loss": -1.0,
        ...
        "metrics": metrics,
        "latest_match": {
            "replay_url": "./data/replay-latest.json",
    ...
    (args.site_data / "replay-latest.json").write_text(
        json.dumps(replay, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    print(
        json.dumps(
            {"status": "PASS", "metrics": metrics, "replay": status["latest_match"]},
            indent=2,
        )
    )
    return 0
