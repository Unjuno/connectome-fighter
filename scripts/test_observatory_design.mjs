...
await test('no evaluations: no fabricated video, spikes, or zero-valued metrics', async ({ page, state }) => {
  ...
  await page.goto(base); assert.equal(await page.locator("html").getAttribute("lang"), "en"); await page.getByText('Awaiting an evaluation video', { exact: true }).waitFor();
...
await test('evaluation failure: explicit error state', async ({ page, state }) => {
  ...
  await page.goto(base); assert.equal(await page.locator("html").getAttribute("lang"), "en"); await page.getByText(/Evaluation API unavailable/).waitFor();
  assert.equal(await page.locator('video').count(), 0);
});
