...
from arena_decision_snapshot import DecisionHistory, SnapshotError, selected_snapshot
...
class ArenaState:
    ...
    def _brain_sample(event: dict[str, Any] | None) -> dict[str, Any]:
        ...
        for row in (brain.get("output_contributions_top") or [])[:20]:
            if not isinstance(row, (list, tuple)) or len(row) < 3:
                continue
            try:
                output_contributions.append({"group": str(row[0]), "body_id": int(row[1]), "spikes": float(row[2])})
            except (TypeError, ValueError):
                continue
        membrane_summary = {}
        ...
        return {
            ...
            "neuromeres": [], "superclasses": [], "types": [], "top_bodies": top_bodies,
        }
    ...
    def payload(self) -> dict[str, Any]:
        ...
        def fighter(side: int, event: dict[str, Any] | None, row: dict[str, Any]) -> dict[str, Any]:
            return {
                "character": self.characters[side], "hp": int(row.get("hp", self.max_hp)),
                "max_hp": self.max_hp, "x": float(row.get("x", 0)), "y": float(row.get("y", 0)),
        ...
        return {
            "schema_version": 3, "session_id": self.session_id, "status": status,
            "round": round_id, "rounds_per_session": self.rounds_per_session,
            "frame": frame, "t_seconds": frame / 60.0,
            "p1": fighter(1, e1, p1_display), "p2": fighter(2, e2, p2_display),
            "brain": {"p1": self._brain_sample(e1), "p2": self._brain_sample(e2)},
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            if path == "/health":
                ...
                return
            if path == "/decision-snapshot":
                try:
                    value = selected_snapshot(state, self.path)
                    code = HTTPStatus.OK
                except SnapshotError as error:
                    code = error.status
                    value = {"ready": False, "status": "snapshot-unavailable", "reason": str(error)}
                self._headers(code, "application/json; charset=utf-8")
                self.wfile.write(json.dumps(value, separators=(",", ":"), allow_nan=False).encode())
                return
            if path == "/state":
                self._headers(HTTPStatus.OK, "application/json; charset=utf-8")
...
def main() -> int:
    ...
    fightingice = subprocess.Popen([
        "java", "-cp", classpath, "Main", processing_flag, "--pyftg-mode", "--input-sync",
        "--limithp", str(args.max_hp), str(args.max_hp), "--port", str(args.game_port),
        "-r", str(rounds_per_session), "-f", "600",
    ], cwd=game_dir, stdout=log_handle, stderr=subprocess.STDOUT, text=True)
    ...
    try:
        ...
        command = [
            sys.executable, str(Path(__file__).with_name("run_game_malecns_lif.py")),
            ...
            "--reference-python", args.reference_python, "--reference-model", str(args.reference_model),
            "--adapter-dir", str(args.adapter_dir), "--interface", str(args.interface),
            "--decision-interval", str(args.decision_interval), "--games", "1",
            "--expected-rounds", str(rounds_per_session), "--timeout", str(timeout_seconds),
            "--run-id", args.session_id, "--out", str(run_root), "--live-telemetry-jsonl", str(telemetry_path),
        ]
        ...
        if code != 0:
            log_handle.flush()
            print(json.dumps({"kind": "arena-runner-exit", "exit_code": code, "error": error,
                              "fightingice_log_tail": log_tail(game_log)}, separators=(",", ":")),
        ...
        code = 1
        log_handle.flush()
        print(json.dumps({"kind": "arena-runtime-error", "error": message,
                          "fightingice_log_tail": log_tail(game_log)}, separators=(",", ":")),
              file=sys.stderr, flush=True)
        hold_observable(args.post_fight_seconds, lambda: stopping)
    finally:
