"""Run one canonical FightingICE match with two independent MaleCNS+Shiu LIF brains.

There is no learned artificial neural network in this control path. Each
FightingICE character owns a separate Brian2 worker process with independent
membrane/synaptic state and RNG. Both workers read the same immutable MaleCNS
structural tables and the same fixed, versioned artificial game interface.
"""
...
def main() -> int:
    ...
    for required in [args.reference_model, args.worker_script, args.interface,
                     args.adapter_dir / "manifest.json",
    ...
    try:
        ...
        characters = [args.character_p1, args.character_p2]
        seeds = [args.seed_p1, args.seed_p2]
        for i, (character, seed) in enumerate(zip(characters, seeds), start=1):
            version = f"malecns-shiu-{character.lower()}-{interface_hash[:10]}-seed{seed}"
