...
def reward_sequence(round_row:dict,side:int,reward_cfg:dict)->list[dict]:
    if reward_cfg.get('id') == 'R3-colosseum-v1':
        from connectome_fighter.colosseum_reward import reward_sequence as colosseum_sequence
        return colosseum_sequence(round_row, side, reward_cfg)
