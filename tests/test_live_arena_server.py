...
def load_module():
    ...
    # Reproduce a script entrypoint's sibling import path for helper modules.
...
def test_brain_sample_preserves_action_group_spike_competition_and_causal_edges():
    ...
    event = {
        ...
        "brain": {
            ...
            "group_spike_counts": {"FORWARD": 2, "BACKWARD": 8, "UP": 1, "DOWN": 3, "A": 0, "B": 17, "C": 4},
    ...
    assert sample["sensory_drive"] == [{"body_id": 111, "rate_hz": 150.0}, {"body_id": 222, "rate_hz": 75.0}]
    assert sample["output_contributions"] == [{"group": "B", "body_id": 333, "spikes": 9.0}, {"group": "BACKWARD", "body_id": 444, "spikes": 6.0}]
    assert sample["membrane_summary"] == {"v_mean_mV": -50.2, "g_mean_mV": 1.4}
    assert sample["top_bodies"] == [{"body_id": 123, "spikes": 11}, {"body_id": 456, "spikes": 7}]


def test_brain_sample_rejects_malformed_spectator_values():
    module = load_module()
    event = {"frame": 60, "brain": {"group_spike_counts": {"B": 5, "bad": "not-a-number"},
            "sensory_drive_top": [[123, 90.0], ["bad"]], "output_contributions_top": [["B", 456, 4.0], ["bad"]],
...
def test_rounds_per_session_override_is_bounded(monkeypatch):
    ...
    with pytest.raises(ValueError): module.rounds_per_session_from_env()
