...
function renderRounds(clip) {
  ...
  for (const row of rounds) {
    ...
    pill.textContent = `R${row.round ?? '?'} · HP ${hp} · reward ${rewards}`;
...
function renderBrain(side, summary, clip) {
  ...
  if (!regions.length) {
    ...
    for (const item of regions.slice(0, 6)) {
      const line = document.createElement('div');
      line.className = 'bar-row';
      const fraction = Math.max(0, Math.min(1, Number(item.fraction || 0)));
...
async function refresh() {
  try {
    ...
    let queue;
    try {
      queue = await getJSON('/live-data/spectator-queue.json');
    } catch (_) {
